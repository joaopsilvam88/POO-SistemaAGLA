...
public class CadastraProfessor extends JDialog implements ActionListener{
	...
	public CadastraProfessor(Sistema sistema){
		try {
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == botao1){
			...
			prof.getDadosPessoais().setNome(caixa.getText());
		...
		if(e.getSource() == botao2){
			janela.setVisible(false);
			prof.getDadosPessoais().setCpf(janela.getText());
		...
		if(e.getSource() == botao3){
			prof.getDadosPessoais().setRg(caixa.getText());
		...
		if(e.getSource() == botao4){
			prof.setMatricula(caixa.getText());
		...
		if(e.getSource() == botao5){
			prof.getDadosPessoais().setEmail(caixa.getText());
			botao5.setVisible(false);
			caixa.setVisible(false);
			setVisible(false);
			try {
				sistema.addProfessor(prof.getDadosPessoais().getCpf(), prof);
				rotulo.setIcon(new ImageIcon(getClass().getResource("Cadastrar.png")));
				setVisible(true);
